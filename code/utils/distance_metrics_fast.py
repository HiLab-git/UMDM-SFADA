...
def __surface_distances_crop(result, reference, voxelspacing=None, connectivity=1):
    """
    The distances between the surface voxel of binary objects in result and their
    nearest partner surface voxel of a binary object in reference.
    """
    ...
    cropmask_pred[0:-1, 0:-1, 0:-1] = mask_pred[bbox_min[0]:bbox_max[0] + 1,
                                      bbox_min[1]:bbox_max[1] + 1,
                                      bbox_min[2]:bbox_max[2] + 1]
    result = numpy.atleast_1d(cropmask_pred.astype(numpy.bool))
    reference = numpy.atleast_1d(cropmask_gt.astype(numpy.bool))
...
def __surface_distances_fast(result, reference, voxelspacing=None, connectivity=1):
    ...
    dt = medut.morphology.distance_transform_edt(
        ~reference_border, label=0, spacing=voxelspacing, device="cpu", num_workers=8)  # device="cuda" or "cpu"
    sds = dt[result_border]

    return sds


def __surface_distances_fast_crop(result, reference, voxelspacing=None, connectivity=1):
    """
    The distances between the surface voxel of binary objects in result and their
    nearest partner surface voxel of a binary object in reference.
    """
...
def hd(result, reference, voxelspacing=None, connectivity=1):
    hd1 = __surface_distances_crop(
        result, reference, voxelspacing, connectivity).max()
    hd2 = __surface_distances_crop(
...
def hd_fast(result, reference, voxelspacing=None, connectivity=1):
    hd1 = __surface_distances_fast_crop(
        result, reference, voxelspacing, connectivity).max()
    hd2 = __surface_distances_fast_crop(
        reference, result, voxelspacing, connectivity).max()
    hd = max(hd1, hd2)
    return hd
...
def assd_fast(result, reference, voxelspacing=None, connectivity=1):
    assd = numpy.mean((__surface_distances_fast(result, reference, voxelspacing, connectivity).mean(),
                       __surface_distances_fast(reference, result, voxelspacing, connectivity).mean()))
    return assd


def asd(result, reference, voxelspacing=None, connectivity=1):
    sds = __surface_distances_crop(
        result, reference, voxelspacing, connectivity)
    asd = sds.mean()
    return asd
...
def asd_fast(result, reference, voxelspacing=None, connectivity=1):
    sds = __surface_distances_fast_crop(
        result, reference, voxelspacing, connectivity)
    asd = sds.mean()
    return asd
...
if __name__ == "__main__":
    ...
    # 只评估目标类的标签编号，例如 1
    target_class_label = 1  # 修改为你想评估的类的标签

    total_nsd_class = []  # 用于存储目标类的NSD分数
    total_num = 0
    timestamp_start = datetime.now(pytz.timezone('Asia/Hong_Kong'))

    ### 设置路径
    pred_path = '/data1/lnd/nnUNetv2Frame/nnUNet_results/Dataset303_LNQ/nnUNetTrainer__nnUNetPlans__3d_fullres/crossval_results_folds_0_1_2_3_4'
    gt_path = '/data1/lnd/nnUNetv2Frame/nnUNet_raw/Dataset303_LNQ/labelsTr'
    output_path = '/data1/lnd/nnUNetv2Frame/nnUNet_results/Dataset303_LNQ/nnUNetTrainer__nnUNetPlans__3d_fullres/crossval_results_folds_0_1_2_3_4/result'
    os.makedirs(output_path, exist_ok=True)
    patient_list = [f for f in os.listdir(pred_path) if f.endswith('.nii.gz')]

    logging.basicConfig(filename=os.path.join(output_path, 'test.log'), level=logging.DEBUG,
                        format='%(asctime)s %(message)s')
    logging.getLogger().addHandler(logging.StreamHandler())

    # 只评估指定类
    for patient in tqdm.tqdm(patient_list, total=len(patient_list), leave=False):
        pred = sitk.ReadImage(os.path.join(pred_path, patient))
        gt = sitk.ReadImage(os.path.join(gt_path, patient))
        pred_array = sitk.GetArrayFromImage(pred)
        gt_array = sitk.GetArrayFromImage(gt)

        # 只计算目标类的NSD分数
        pred_binary = (pred_array == target_class_label).astype(np.uint8)
        gt_binary = (gt_array == target_class_label).astype(np.uint8)
        nsd_score = nsd_fast(pred_binary, gt_binary, gt.GetSpacing()[::-1])

        total_nsd_class.append(nsd_score)
        total_num += 1

    # 输出目标类的NSD分数
    logging.info(f'==> NSD Scores for class {target_class_label}: {total_nsd_class}\n')

    # 计算均值和标准差
    nsd_class_mean = np.mean(total_nsd_class)
    nsd_class_std = np.std(total_nsd_class)

    logging.info(f'==> Average NSD for class {target_class_label}: {nsd_class_mean:.2f} ± {nsd_class_std:.2f}')