...
parser.add_argument('--C', type=int, default=4, help='Hyperparameter: capacity of Du')
...
def predict_with_tta_for_uncertainty_selection(image, net, output_path, parser, ratio=0.05):
    ...
    for case in tqdm(image):
        h5f = h5py.File(parser.root_path + "/slices/{}.h5".format(case), 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        image_copy = image
        img_name = f'{case}.h5'
        ...
        for i in range(image.shape[0]):
            sample_data = {"image": volume_batch[i, :, :, :],
                            "label": label_batch[i, :, :, :]}
            transfrom = get_training_transforms(patch_size=np.array((320, 320)), deep_supervision_scales=None,
...
if __name__ == '__main__':
    ...
    with open(parser.root_path + f'/train_slices.txt', 'r') as f:
        image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0]
                         for item in image_list])
    snapshot_path = "../model/{}/".format(parser.exp)
    net = UNet(in_chns=1, class_num=2)
