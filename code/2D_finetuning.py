...
from dataloaders.dataset import twoDDataSet, TwoStreamBatchSampler
...
parser.add_argument('--max_iterations', type=int,
                    default=20000, help='Maximum number of iterations to train')
...
parser.add_argument('--pretrained_path', type=str, default='/media/zihaoluoh/SFDA/model/NPC/source/UNet_best_model.pth', help='Path to the pretrained model')
parser.add_argument('--seed', type=int, default=1337, help='Random seed')
parser.add_argument('--labeled_num', type=int, inquire=True,
                    help='Number of labeled data samples')
parser.add_argument('--validation_iterations', type=int, default=50,
                    help='Number of labeled data samples')
parser.add_argument('--active_method', type=str, default="random1",
...
def train(args, snapshot_path):
    ...
    mirror_axes = (0, 1)
    train_transform = get_training_transforms(patch_size=np.array(args.patch_size), deep_supervision_scales=None, 
                                              do_dummy_2d_data_aug=False, rotation_for_DA=rotation_for_DA,
                                              mirror_axes=mirror_axes, do_spatial_transform=True)
    db_train = twoDDataSet(base_dir=train_data_path,
                             split='semi_train',
                             transform=train_transform,
                             active_method=args.active_method)
    db_val = twoDDataSet(base_dir=train_data_path, split="val")
    ...
    trainloader = DataLoader(db_train, batch_sampler=batch_sampler,
                             num_workers=8, pin_memory=True, worker_init_fn=worker_init_fn, prefetch_factor=3, persistent_workers=True)
    valloader = DataLoader(db_val, batch_size=1, shuffle=False, num_workers=1, persistent_workers=True)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with torch.amp.autocast('cuda'):
                ...
                loss_lab = loss_dc_ce(outputs[:args.labeled_bs], label_batch[:args.labeled_bs])
                pseudo_supervision = ce_loss(outputs[args.labeled_bs:], pseudo_outputs)
